...
def demosaic(white_balanced_image, cfa_pattern, output_channel_order='BGR', alg_type='VNG', device='0'):
    """
    ...
    elif alg_type == 'down':  # G R B G
        demosaiced_image = np.zeros([wb_image.shape[0]//2, wb_image.shape[1]//2, 3], dtype=np.float32)
        demosaiced_image[:,:,0] = wb_image[0::2, 1::2]
        demosaiced_image[:,:,1] = wb_image[0::2, 0::2]
        demosaiced_image[:,:,2] = wb_image[1::2, 0::2]
    elif alg_type == 'net':
        cfa_pattern_str = "".join(["RGB"[i] for i in cfa_pattern])
        bayer = np.power(np.clip(wb_image.astype(dtype=np.float32) / max_val, 0, 1), 1 / 2.2)
        pretrained_model_path = os.path.dirname(__file__) + "/model.bin"
        demosaic_net = demosaic_bayer.get_demosaic_net_model(pretrained=pretrained_model_path, device=device, 
                                                             cfa='bayer', state_dict=True)
        rgb = demosaic_bayer.demosaic_by_demosaic_net(bayer=bayer, cfa=cfa_pattern_str, 
                                                      demosaic_net=demosaic_net, device=device)
        demosaiced_image = np.power(np.clip(rgb, 0, 1), 2.2) * max_val
