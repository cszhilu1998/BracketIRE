...
class RealModel(BaseModel):
    ...
    def __init__(self, opt):
        super(RealModel, self).__init__(opt)
        ...
        self.load_network_path(self.netTMRNet, './ckpt/syn/TMRNet_model_400.pth')
        ...
        self.load_network_path(self.netTMRNetPre, './ckpt/syn/TMRNet_model_400.pth')
    ...
    def forward_chop(self, data_raws, chop_size=800):
        ...
        out = torch.zeros([1, c, new_h, new_w], dtype=torch.float32, device=data_raws.device)
        for i in range(num_h):
            for j in range(num_w):
                out[:, :, i*chop_size:(i+1)*chop_size, j*chop_size:(j+1)*chop_size] = self.netTMRNet(
...
class TMRNet(nn.Module):
    ...
    def upsample(self, lqs, feats, base_feat):
        ...
        for t in t_list:
            ...
            hr = self.reconstruction(hr) + skip1
            hr = self.conv_last(self.lrelu(self.conv_hr(hr)))
            out.append(hr)
