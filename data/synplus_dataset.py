...
# BracketIRE+ dataset
class SynPlusDataset(BaseDataset):
    def __init__(self, opt, split='train', dataset_name='BracketIREPlus'):
        super(SynPlusDataset, self).__init__(opt, split, dataset_name)
    ...
    def _getitem_val(self, idx):   
        ...
        return {'meta': meta,
                'gt': gt[..., 4*self.scale:(128+4)*self.scale, 4*self.scale:(128+4)*self.scale],
                'raws': raws[..., 4:128+4, 4:128+4],
                'fname': self.names[idx]}
