...
def safe_invert_gains(image, rgb_gain, red_gain, blue_gain, device):
    ...
    gains = torch.stack((1.0 / red_gain, torch.tensor([1.0]).to(device), 1.0 / blue_gain)) # / rgb_gain
...
def random_noise_levels(noise_level):
...
# def random_noise_levels(noise_level):
#   """ Where read_noise in SIDD is not 0 """
#   log_min_shot_noise = np.log(0.00068674)
#   log_max_shot_noise = np.log(0.02194856)
#   # log_shot_noise     = torch.FloatTensor(1).uniform_(log_min_shot_noise, log_max_shot_noise)
#   log_shot_noise = torch.FloatTensor([log_min_shot_noise + noise_level * (log_max_shot_noise - log_min_shot_noise)])
#   shot_noise = torch.exp(log_shot_noise)

#   line = lambda x: 1.85 * x + 0.30
#   n    = tdist.Normal(loc=torch.tensor([0.0]), scale=torch.tensor([0.20])) 
#   log_read_noise = line(log_shot_noise) + n.sample()
#   read_noise     = torch.exp(log_read_noise)
#   return shot_noise, read_noise